import { finalizeConversationSelection, SelectionValidationError, } from './selection-normalizer.js';
...
function acceptedTextNodes(anchor, root) {
    const nodes = [];
    const walker = document.createTreeWalker(anchor, NodeFilter.SHOW_TEXT);
    let current = walker.nextNode();
    while (current !== null) {
        const text = current;
        if (!excluded(text, root))
            nodes.push(text);
        current = walker.nextNode();
    }
    return nodes;
}
function selectedSlice(range, node) {
...
export async function captureDomConversationSelection(input) {
    ...
    if (!input.conversationRoot.contains(range.startContainer)
        ...
        throw new SelectionValidationError('selection_outside_conversation', 'The selection must stay inside the current conversation.');
    }
    ...
    if (startAnchor === null || endAnchor === null || startAnchor !== endAnchor) {
        throw new SelectionValidationError('selection_crosses_unsupported_nodes', 'This compatibility adapter supports a selection inside one message only.');
    }
...
function anchorByKey(root, nodeKey) {
    return [...root.querySelectorAll('[data-chat-anchor-key]')]
...
function restoreFragmentRanges(root, fragment) {
    ...
    for (const node of nodes) {
        ...
        if (visibleOffset >= fragment.endOffset)
            break;
    }
